...
runcode<-function(codelist, # merged dataset of loa and configuration file
  ...
  head12<-paste0('\n  pageW<-11 \n  pageH<-8.5 ',
                 '\n  figW<-9 #figure width in RTF file',
                 '\n  figH<-6 #figure height in RTF file\n',
  ...
  for (i in 1:nrow(codelist)){
    ...
    if( (!is.na(codelist$Type.y[i]) & codelist$Type.y[i]=='Figure') || 
      ...
      addplot[4]<-paste0('  addPlot(rtf,pngtmp,width=figW', #codelist$width[i]/160,
                         ',height=figH'#,codelist$height[i]/160,')'
                         )  
