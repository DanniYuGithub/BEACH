...
#install packages
if(TRUE){
  dep.packages <- c("shiny", "DT", "haven", "xtable", "rtf", "plyr", "sas7bdat",
                    "WriteXLS",  #"rJava",
                     "devtools");
  ...
  dPAI_load <<- 0
  #1 if dPAI is activated at least once.
  
  userActionLabel <<- 'user click'
  userdo_action <<- function(){return(NULL)}
  #predefined global value for user do action button.
...
if (TRUE){
  ...
  #keyword in widges
  wid.keywd<<-c('check','radio','dropdown','slide', 
  ...
  BeachRead<<-function(file, header=TRUE, nrow=-1,  
    ...
    if(is.sas){
      ...
      if(class(ot.t)[1]=='try-error' && 
        ...
        library(sas7bdat.parso)
        print(ot.t)
        ot.kk<<-try(ot<<-sas7bdat.parso::read.sas7bdat.parso(file))
      }
      if(#class(ot.kk)[1]=='try-error' || 
        is.null(ot)){
...
BeachServer <- function(input,output, session){
  ...
  output$setTitle<-renderUI( try( {
    ...
    if(nrow(r123<-VdicTitle())>0){
      ...
      s1s<-r123[ as.numeric(r123$Num)%in%c(0,1),'Source']
      for(s1 in s1s){
  ...
  output$setconfig<-renderUI({
    ...
    #get a map between LOA and CD for widgets
    #slide.value  radio.choice  check.choice dropdown.choice   
    #date.value   dateR.value    text.value
    m1<-params.id$m1; m2<-params.id$m2; m2[m2==0]<-'';
    m3<-rep('value', length(m1))
    m3[m1%in%c('check', 'radio', 'dropdown')]<-'choice'
    params.map<<-data.frame(loa_lab=params.lab,
                           cd_lab=paste0(m1,'.label', m2),
                           loa=paste0(m1,m2),
                           cd=paste0(m1,'.',m3,m2))
  ...
  output$getData<-renderUI({#Declare global variables#
    ...
    tm.Vdic<-Vdic()
    if(!is.na(tm.Vdic$select.label[1])) #load source code in 1st available row
    ...
    for (i in AnalyN()){
      ...
      if(Vdic()$Type[i]=='Figure'){ 
        if(length(input0.code)>0 && grepl("dynamicData", input0.code, fixed=TRUE)){
          ...
          if(FALSE){
            tmpcode<-paste0("<div class=\"row\">", "\n",
                            "<div class=\"col-sm-12\">", "\n",
                            #"<div id=\"dynamicPlot\" class=\"shiny-plot-output\" style=\"width:",input$figSize*100,"% ; height:",100*input$figSize,"%\" ",
                            "<div id=\"dynamicPlot\" class=\"shiny-plot-output\" style=\"width:100%; height=400px\" ",
                            "data-click-id=\"dynamicPlot_click\" data-click-clip=\"TRUE\" ",
                            "data-brush-id=\"dynamicPlot_brush\" data-brush-fill=\"#9cf\" data-brush-stroke=\"#036\" ",
                            "data-brush-opacity=\"0.25\" data-brush-delay=\"300\" data-brush-delay-type=\"debounce\" ",
                            "data-brush-clip=\"TRUE\" data-brush-direction=\"xy\" data-brush-reset-on-new=\"FALSE\"></div>",
                            "\n", "</div></div>", "\n",
                            "<div class=\"row\">", "\n",
                            
                            "<div class=\"col-sm-12\">", "\n",
                            "<a id=\"click_info_dl\" class=\"btn btn-default shiny-download-link\" href=\"\" target=\"_blank\">", 
                            "\n",
                            ...
                            "\n",
                            "<pre id=\"click_info_DT\" class=\"shiny-html-output\"></pre>", "\n",
                            "</div>", "\n", 
                            
                            "<div class=\"col-sm-12\">", "\n", 
                            "<a id=\"brush_info_dl\" class=\"btn btn-default shiny-download-link \" href=\"\" target=\"_blank\">", 
                            "\n",
                            "<i class=\"fa fa-download\"></i>", 
                            "Save Brushed Data", 
                            "</a>", 
                            ...
                            "\n", 
                            "<pre id=\"brush_info_DT\" class=\"shiny-html-output\"></pre>", "\n",
                            "</div>", "\n", "</div>", "\n")
          }else{
  ...
  output$AnalysisTab<-renderUI({
    ...
    conditionalPanel(condition=cpCon, width='100%',
                     ...
                     div(class='span12', 
                         uiOutput("TFL"), 
                         uiOutput("footnote"), 
                         #actionButton("userdo", userActionLabel), 
                         uiOutput('userdo_button'),
                         verbatimTextOutput("userdo_out"),
                         br(), 
  ...
  output$save_loa_cd<-downloadHandler(
    filename=function(){paste0(input$study,"_", input$tumor,"_loa_cd.csv")},
    content=function(file){
      f12<-function(d1,d2){
        n2<-nrow(d2)
        d2$FootCode <- d2$Footnote
        d2$StatNote <- d2$Test.Statistic.Footnote
        d2$StatModel <-d2$Statistical.Analysis
        d2m<-params.map[,c('loa_lab','cd_lab')]
        d2cm<-d2m$loa_lab; names(d2cm)<-d2m$cd_lab;
        d2mv<-params.map[,c('loa','cd')]
        d2cmv<-d2mv$loa; names(d2cmv)<-d2mv$cd;
        for(i in 1:length(d2cm)){
          colnames(d2)<-gsub(d2cm[i], names(d2cm)[i], colnames(d2), fixed=T)
          colnames(d2)<-gsub(d2cmv[i], names(d2cmv)[i], colnames(d2), fixed=T)
        }
        dcm<-colnames(d2)[colnames(d2)%in%colnames(d1)]
        dcm.txt<-dcm[grepl('text.value',dcm)]
        dcm.ntx<-dcm[!grepl('text.value',dcm)]
        d2$rn<-1:n2
        d2$rn2<-paste(d2$rn, d2$Request.Name)
        d12<-merge(d1, d2[,c('rn','Request.Name', 'rn2')], 
                   by='Request.Name', all=T, sort=F)
        d12$Num[d12$Num!=0] <- NA
        d12$Num[d12$Tab.value=='cd']<-n2+1
        d12$Num[d12$Tab.value=='rcode']<-n2+2
        d12 <- d12[d12$Num%in%c(0, n2+1, n2+2)|!is.na(d12$rn),]
        d12$Num[!is.na(d12$rn)]<-d12$rn[!is.na(d12$rn)]
        for(i in d2$rn){
          d12[d12$Num==i, dcm.ntx]<-d2[d2$rn==i,dcm.ntx]
          dcm.txt.tmp <- d2[d2$rn==i,dcm.txt]
          d12[d12$Num==i, dcm.txt]<-ifelse(is.na(dcm.txt.tmp),
                                           NA,
                                           paste0('\"',dcm.txt.tmp,'\"'))
        }
        d12$Request.Name<-d12$rn2
        d12<-d12[,colnames(d1)]
        d12<-d12[order(d12$Num),]
        loc.wd<-grepl(',',d12$width, fixed=T)
        d12$width[loc.wd]<-paste0('\"c(',d12$width[loc.wd],')\"')
        rownames(d12)<-NULL
        return(d12)
      }
      tt1<<-Vdic0; tt2<<-LOA()[loaTF(),];
      #f12(Vdic0, tt2);
      write.csv(  f12(Vdic0, LOA()[loaTF(),]) ,
      ...
      for (i in 1:nrow(codelist)){
        ...
        if(!all(is.na(codelist[i,params]))){
          ...
          for (k in k.list[length(k.list):1]){
            print(     tmp0<-unlist(strsplit(as.character(codelist[i,k]),',',fixed=TRUE))   )
  ...
  #---------run user defined action button-----------------#
  output$userdo_button<-renderUI({
    AnalyN()
    but_code<-paste0(
      '<button id="userdo" type="button" class="btn btn-default action-button">',
      userActionLabel, '</button>')
    return(shiny::HTML(but_code))
  })
  #observeEvent(input$userdo, {userdo_action()})
  userdo_out_reac<-eventReactive(input$userdo, {AnalyN(); userdo_action()})
  output$userdo_out<-renderPrint({ 
     if(!is.character(userdo_out_reac())){
       return(NULL)
    }else{return(userdo_out_reac())}
  })
  #---------End of run user defined action button-----------------#
  ...
  output$getDataSelected <- renderUI({
    ...
    if(is.null(input$outTbl_rows_selected)){
      ...
      #input$outTbl_rows_selected is shiny internal widget indicate row id(s)
